"""Darvas box, mirrored to the short side — trade the BREAKDOWN of the box,
which the pine only paints as a red arrow and never trades.

Nicolas Darvas famously never shorted; his boxes were pure long vehicles.
But the box construct itself is direction-neutral — a range with a proven
top and bottom — so this interpretation trades its other edge: park a
resting SELL-stop just under the bottom of every completed box, and let a
breakdown fill it.

Management mirrors the classic file exactly, upside down:

  stop    a buy-stop at the box TOP (the range's proven ceiling)
  trail   each LATER completed box whose top sits LOWER ratchets the cover
          stop down above it — the staircase runs downward
  target  none; the runner lives until a box ceiling gives out

The resting entry lives exactly as long as its box, and box geometry (not R)
anchors every level, so nothing re-anchors on a gapped fill.
"""
...
class DarvasShortStrategy(stonks.Strategy):
    length = 4
    ...
    params = {
        ...
        "entry_buffer_bps": stonks.Param("sell-stop offset below the box bottom", unit="bps"),
    ...
    indicators = {
        "box_top": stonks.Indicator("active Darvas box top (the cover stop)"),
        "box_bottom": stonks.Indicator("active Darvas box bottom (the breakdown level)"),
    }
    ...
    def on_tick(self, ctx):
        ...
        for symbol, sub in df.groupby("symbol", sort=False):
            ...
            if in_pos:
                # Downward staircase: a fresh box with a LOWER top ratchets
                # the cover stop down above it.
                if new_box and st["bx_high"] < st["stop_px"]:
                    if st["sl"] is not None:
                        ctx.cancel_order(st["sl"])
                    st["stop_px"] = st["bx_high"]
