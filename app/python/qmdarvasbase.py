"""Qullamaggie x Darvas — the box IS the flag.

QM's tight-flag detector (pivot scan, base age, retracement depth) and the
Darvas 5-state machine are two formalizations of the same object: a
consolidation under a proven high. This hybrid swaps QM's scan out for the
box machine and keeps EVERYTHING else from the QM playbook:

  base        a completed Darvas box (state 5), instead of the pivot scan
  gates       full QM universe at arm time: price floor, ADR floor, momentum
              gain, close above the 20-SMA with the 10-SMA above it
  entry       resting buy-stop just above the box top, alive while the box is
  stop        the TIGHTER of the QM ADR stop and the box bottom
  management  pure QM: half partial at 2R or after partial_bars, breakeven
              move, EMA20 close-trail on the rest; weak-volume fills are
              scratched; gapped fills re-anchor the legs

Contrast with qmdarvasuniverse.py, which keeps Darvas' OWN exit engine and
borrows only QM's universe gates. This file is the opposite composition.
"""
...
class QMDarvasBaseStrategy(stonks.Strategy):
    ...
    params = {
        ...
        "adr_stop_mult": stonks.Param("ADR stop distance (tightened vs the box bottom)", unit="ADR"),
    ...
    indicators = {
        "box_top": stonks.Indicator("active box top (the QM pivot stand-in)"),
        "box_bottom": stonks.Indicator("active box bottom"),
        "trail": stonks.Indicator("trailing EMA the post-partial exit checks"),
    }

    def lookback(self):
        return max(self.length, 51, self.mom_len, self.adr_len,
                   3 * self.trail_len) + 5
    ...
    def on_tick(self, ctx):
        ...
        for symbol, sub in df.groupby("symbol", sort=False):
            ...
            # The resting entry lives as long as its box, like darvasclassic.
            ...
            if st["pending"] is None and new_box and self._universe_up(hi, lo, cl, vo):
                ...
                stop_px = max(adr_stop, float(st["bx_low"]))       # the tighter of the two
                stop_px = min(stop_px, entry_px * 0.999)
                target_px = entry_px + self.partial_rr * (entry_px - stop_px)
